...
async def upload_file(background_tasks: BackgroundTasks, file: UploadFile = File(...), user_id: str = Form(...), title: str = Form(...)):
    try:
        file_bytes = await file.read()
        if len(file_bytes) > 25 * 1024 * 1024: raise HTTPException(status_code=400, detail="File exceeds 25MB limit")
        ext = file.filename.split('.')[-1].lower() if file.filename else ""
        if ext in ['pdf', 'pptx', 'docx']: source_type = ext
        elif ext in ['mp3', 'mp4', 'wav', 'm4a', 'webm', 'mpeg', 'mpga']: source_type = 'audio' if ext in ['mp3', 'wav', 'm4a', 'mpeg', 'mpga'] else 'video'
        else: raise HTTPException(status_code=400, detail="Unsupported file format")

        supabase = get_supabase_client()
        file_path = f"{user_id}/{file.filename}"
        try: supabase.storage.from_("documents").upload(path=file_path, file=file_bytes, file_options={"upsert": "true"})
        except Exception: pass

        result = supabase.table("sources").insert({"user_id": user_id, "title": title, "source_type": source_type, "file_path": file_path, "status": "processing"}).execute()
...
async def ingest_web(request: WebIngestRequest, background_tasks: BackgroundTasks):
    try:
        supabase = get_supabase_client()
        result = supabase.table("sources").insert({"user_id": request.user_id, "title": request.title or request.url, "source_type": request.type, "youtube_url": request.url, "status": "processing"}).execute()
        source_id = result.data[0]["id"]
...
async def ingest_text(request: TextIngestRequest, background_tasks: BackgroundTasks):
    try:
        supabase = get_supabase_client()
        result = supabase.table("sources").insert({"user_id": request.user_id, "title": request.title, "source_type": "text", "status": "processing"}).execute()
        source_id = result.data[0]["id"]
...
def process_file(source_id, user_id, file_bytes, source_type, filename):
    try:
        ...
        pages = parser.parse_audio_video(file_bytes, filename) if source_type in ['audio', 'video'] else getattr(parser, f"parse_{source_type}")(file_bytes)
        run_pipeline(source_id, user_id, pages)
    except Exception as e: mark_error(source_id, str(e))
...
def run_pipeline(source_id, user_id, pages):
    ...
    rows = [{"source_id": source_id, "user_id": user_id, "content": c["content"], "chunk_index": c["chunk_index"], "page_number": c.get("page_number"), "embedding": e} for c, e in zip(chunks, embeddings)]
    for i in range(0, len(rows), 50): supabase.table("chunks").insert(rows[i:i+50]).execute()
