...
interface AppSidebarProps { userEmail: string; userId: string; }
...
export function AppSidebar({ userEmail, userId }: AppSidebarProps) {
  ...
  return (
    <motion.aside
      ...
      className="relative flex flex-col h-screen border-r border-border/50 bg-background/50 backdrop-blur-xl z-40"
    >
      {/* HIGLY VISIBLE TOGGLE BUTTON */}
      <motion.button
        onClick={() => setCollapsed(!collapsed)}
        className="absolute -right-4 top-8 z-50 flex h-8 w-8 items-center justify-center rounded-full border border-border bg-background shadow-md hover:bg-accent text-foreground transition-colors"
            ...
            <Link key={navItem.href} href={navItem.href}>
              <motion.div
                ...
                className={cn(
                  ...
                  isActive ? "text-primary-foreground shadow-md shadow-primary/20" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                )}
              >
                {isActive && (
                  <motion.div layoutId="sidebar-active" className="absolute inset-0 bg-primary rounded-xl" transition={{ type: "spring", stiffness: 300, damping: 30 }} />
                )}
